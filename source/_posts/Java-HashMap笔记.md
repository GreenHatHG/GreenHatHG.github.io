...
从另外一个角度来讲，`HashMap`不是线程同步的，并且在不添加额外的同步的代码情况下是不能被别的线程访问的。我们可以使用`Collections.synchronizedMap()`方法去生成线程安全的`HashMap`。当然我们可以使用锁或者是用

`synchronized`关键字让`HashMap`变成线程安全的
